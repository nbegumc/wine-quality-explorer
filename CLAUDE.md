...
```bash
uv run train.py                                         # full experiment; regenerates dist/results.json, dist/data.json, data/selected-network.bif, data/holdout-predictions.csv
...
uv run serve.py                                         # dashboard at http://localhost:8000 (serves dist/ statically)
uv run package_download.py                              # builds dist/wine-quality-python.zip
```
...
Two halves connected by one JSON contract (`dist/results.json`):
...
- `dist/inference.mjs` — variable elimination over the exported factors. Must stay numerically identical to pyAgrum; `tests/test_inference.mjs` checks this against `reference_queries` that `train.py` writes into `results.json`.
- `dist/app.mjs` — dashboard UI; reads `results.json` and `data.json` only.
...
- `docs/BEGINNER_GUIDE.md`, `docs/LEARNING_GUIDE.md`, and the PDF/DOCX in `dist/` are user-facing teaching material that quote specific numbers from the results table; regenerating results means updating them.
